  ...
  return (
    <div className="content-container py-8 small:py-12">
      ...
      <section className="py-8 small:py-10">
        <LocalizedClientLink href={`/journal/${featuredPost.slug}`} className="block">
            ...
            <div className="grid gap-0 lg:grid-cols-[1.05fr_0.95fr]">
              ...
              <div className="p-6 small:p-8">
                ...
                <div className="mt-6 flex flex-wrap gap-2 text-sm text-[var(--shreem-muted)]">
                  <span className="brand-pill px-3 py-1.5">{featuredPost.readTime}</span>
                  <span className="brand-pill px-3 py-1.5">{featuredPost.publishedAt}</span>
                </div>
                <div className="mt-8">
                  <span className="brand-primary-button">Read featured note</span>
                </div>
              </div>
            </div>
          </article>
      ...
      <section className="pb-16 pt-2 small:pb-24">
        <div className="grid gap-4 xl:grid-cols-2">
          {posts.map((post) => (
            <LocalizedClientLink key={post.slug} href={`/journal/${post.slug}`} className="block">
              <article className="brand-card overflow-hidden h-full">
